...
| `mise`                | Per-project runtime versions (node, python, go, …) from `mise.toml` / `.tool-versions` — [jdx/mise](https://github.com/jdx/mise)                                                                                                                                                                                                                                                                            | `mise` (`brew`, `pacman`); else `git` + `cargo` fallback (needs Rust)                   | zsh, bash | `mode` (`enum` `activate` \| `shims`, default `activate`)                                                                                 |
| `pay-respects`        | Suggest a corrected command after a failed one, run via the alias (a Rust `thefuck` alternative) — [iffse/pay-respects](https://github.com/iffse/pay-respects)                                                                                                                                                                                                                                              | `pay-respects` (`brew`); else `git` + `cargo` fallback (needs Rust)                     | zsh, bash | `alias` (string, default `f`)                                                                                                             |
| `atuin`               | SQLite-backed shell history with fuzzy search, context and stats — [atuinsh/atuin](https://github.com/atuinsh/atuin). **Mutually exclusive with `fzf`** (both bind Ctrl+R) — enabling both is a config error.                                                                                                                                                                                               | `atuin` (`brew`, `pacman`); else `git` + `cargo` fallback (needs Rust)                  | zsh, bash | `bind_ctrl_r` (bool, default `true`), `bind_up_arrow` (bool, default `true`)                                                              |
...
| `root-loops`          | Recolour the terminal to a [rootloops.sh](https://rootloops.sh)-generated 16-colour palette on shell start via OSC 4/10/11 escapes (tmux-aware). **Opt-in: it overrides your terminal's own colours.** `auto` follows the system light/dark setting (`AppleInterfaceStyle` on macOS, the freedesktop `color-scheme` portal on Linux).                                                                       | — (config only)                                                                         | zsh, bash | `appearance` (`enum` `auto` \| `dark` \| `light`, default `auto`)                                                                         |
...
- Every write to a rc file or init file is preceded by a timestamped backup
  under `~/.config/omnishell/backups/<timestamp>/`.
- `apply` refuses to overwrite an `init.<shell>` file that was edited by hand (exit 1); it tells you to re-run with `--force`. `omnishell doctor` reports
...
- `omnishell rollback` restores files and the lockfile from a backup snapshot (see Commands) — but never packages or vendored files, and never
